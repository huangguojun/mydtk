        ...
        for (vtkIdType i = 0; i < nb_arrays; ++i) {
            ...
            if (nb_cmpts == 1) {
                for (vtkIdType t = 0; t < nb_tuples; ++t) {
                    ...
                    double value = val * std::cos(val + ((cell_bounds[0] -
                    bounds[0])/bounds[1])); array->SetTuple1(t, value);
                    */
                }
            } else if (nb_cmpts == 3) {
                for (vtkIdType t = 0; t < nb_tuples; ++t) {
                    ...
                    /*
                     ...
                     new_value[0] = val * std::cos(val + ((cell_bounds[0] -
                     bounds[0])/bounds[1])); new_value[1] = val * std::cos(val +
                     ((cell_bounds[2] - bounds[2])/bounds[3])); new_value[2] =
                     val * std::cos(val + ((cell_bounds[4] -
                     bounds[4])/bounds[5])); array->SetTuple3(t, new_value[0],
    ...
    : QMainWindow(parent), d(new dtkVisualizationViewerPrivate)
{
    ...
    // dtkWidgetsParameterMenuBarGenerator menubar_generator(":menu.json",
    // ":parameters.json"); menubar_generator.populate(this->menubar());
    // this->populate();

    // this->menubar()->touch();

    //  QHash<QString, dtkCoreParameter *> parameters =
    //  menubar_generator.parameters();
    ...
    //      parameters["open"]->setValue(default_path);
      ...
      if(plotdata_parameter) {
          plotdata_parameter->connect([=] (QVariant v) {
                  dtkVisualizationPlot2D *plot =
  dynamic_cast<dtkVisualizationPlot2D *>(d->stack->layout()->current()->view());
...
void dtkVisualizationViewer::applyScale(double x, double y, double z)
{
    ...
    dtkVisualizationCanvas *canvas =
            dynamic_cast<dtkVisualizationCanvas *>(d->stack->layout()->current()->view());
    if (!canvas)
        return;
...
void dtkVisualizationViewer::importDataSet(const QString &path)
{
    ...
    /*
        ...
        dtkVisualizationDecoratorInfo              *decorator_info = new
       dtkVisualizationDecoratorInfo; dtkVisualizationDecoratorAxes
       *decorator_axes               = new dtkVisualizationDecoratorAxes;
        dtkVisualizationDecoratorDelaunay2D        *decorator_delaunay_2D = new
       dtkVisualizationDecoratorDelaunay2D; dtkVisualizationDecoratorPoints
       *decorator_points             = new dtkVisualizationDecoratorPoints;
        dtkVisualizationDecoratorScalarColorMap    *decorator_scalar_colormap =
       new dtkVisualizationDecoratorScalarColorMap;
        dtkVisualizationDecoratorScalarGlyphs      *decorator_scalar_glyphs =
       new dtkVisualizationDecoratorScalarGlyphs;
        dtkVisualizationDecoratorScalarIsolines    *decorator_scalar_isolines =
       new dtkVisualizationDecoratorScalarIsolines;
        dtkVisualizationDecoratorSlices            *decorator_slices = new
       dtkVisualizationDecoratorSlices; dtkVisualizationDecoratorSnapshot
       *decorator_snapshot           = new dtkVisualizationDecoratorSnapshot;
        dtkVisualizationDecoratorVectorStreamlines *decorator_vector_streamlines
       = new dtkVisualizationDecoratorVectorStreamlines;
        dtkVisualizationDecoratorVolume            *decorator_volume = new
       dtkVisualizationDecoratorVolume;
        ...
        dtkWidgetsMenu *canvas_menu = menu_bar->menu("View " +
       canvas->objectName());
        ...
        if (decorator_info->isDecorating()) {
            for(auto inspector : decorator_info->inspectors()) {
                dtkWidgetsMenuItemDIY *item = new
       dtkWidgetsMenuItemDIY(inspector->objectName(), inspector);
