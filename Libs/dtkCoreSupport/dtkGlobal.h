...
#define DTK_DEFAULT_IMPLEMENTATION                                                                 \
    dtkDebug() << "Using default implementation of" << DTK_PRETTY_FUNCTION << "for"                \
               << this->metaObject()->className()

#define DTK_DEFAULT_IMPLEMENTATION_NO_MOC                                                          \
    dtkDebug() << "Using default implementation of" << DTK_PRETTY_FUNCTION
...
protected:                                                                                         \
    ...
    {                                                                                              \
        return reinterpret_cast<Class##Private *>(dtkAbstractObject::d_func());                    \
    }                                                                                              \
    inline const Class##Private *d_func(void) const                                                \
    {                                                                                              \
        return reinterpret_cast<const Class##Private *>(dtkAbstractObject::d_func());              \
    }                                                                                              \
                                                                                                   \
private:                                                                                           \
...
public:
    enum {
        ...
        dtkAbstractObjectPointer =
                (sizeof(checkAbstractObject(static_cast<T *>(0))) == sizeof(yes_type)),
        dtkMatrixRealPointer =
                (sizeof(checkAbstractMatrix(static_cast<T *>(0))) == sizeof(yes_type))
    };
};
