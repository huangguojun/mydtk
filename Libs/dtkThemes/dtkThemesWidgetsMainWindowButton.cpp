...
dtkThemesWidgetsMainWindowButton::dtkThemesWidgetsMainWindowButton(Kind kind, QWidget *parent) : QFrame(parent), d(new dtkThemesWidgetsMainWindowButtonPrivate)
{
    d->kind = kind;
...
{
    dtkThemesWidgetsMainWindow *window = static_cast<dtkThemesWidgetsMainWindow *>(parentWidget()->parentWidget());

    switch(d->kind) {
    case Maximize:
        window->isMaximized() ? window->showNormal() : window->showMaximized(); break;
    case Minimize:
...
void dtkThemesWidgetsMainWindowButton::paintEvent(QPaintEvent *event)
{
    ...
    switch(d->kind) {
    case Maximize:
        painter.setBrush(dtkThemesEngine::instance()->color("@green")); break;
    case Minimize:
        painter.setBrush(dtkThemesEngine::instance()->color("@orange")); break;
    default:
        painter.setBrush(dtkThemesEngine::instance()->color("@red")); break;
    };

    static const int length { size().height() * 5 / 6 };
    static const QRect rect((size().height() - length) / 2, (size().height() - length) / 2, length, length);
