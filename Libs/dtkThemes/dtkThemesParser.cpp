...
QString dtkThemesParser::parse(const QString &path)
{
    auto eval = [=](const QString &source, bool compound = false) -> QString {
        ...
        qSort(keys.begin(), keys.end(), dtkThemesParserSorter);
    ...
    QRegularExpression s_expression(
            "^;; (?<key>@([a-z])+(\\d)*(\\-)*([a-z])*):\\s*(?<value>#(.)+).*$");
    QRegularExpression c_expression(
            "^;; "
            "(?<key>@([a-z])+(\\d)*(\\-)*([a-z])*):\\s*(?<value>@([a-z])+(\\d)*(\\-"
            ")*([a-z])*).*$");
    ...
    while (stream.readLineInto(&line)) {
        ...
        if (s_match.hasMatch()) {
            ...
            // qDebug() << Q_FUNC_INFO << s_match.captured("key") <<
            // s_match.captured("value");
    ...
    QRegularExpression o_expression(
            "^.*(?<operator>(alpha))\\((?<value>(#([a-z]|[A-Z]|[0-9])+)),\\s*(?<"
            "operand>([a-x]|[0-9])+).*\\).*$");
    ...
    while (o_stream.readLineInto(&line)) {
        ...
        // if(t_match.hasMatch()) {
        //     qDebug() << Q_FUNC_INFO << "Got an OP for:" << path << "in:" <<
        //     line;
        // }

        if (o_match.hasMatch()) {

            // qDebug() << Q_FUNC_INFO << "Strogin an OP substitution for:" <<
            // path << "in:" << line;

            QString s = QString("%1(%2, %3)")
                                .arg(o_match.captured("operator"))
                                .arg(o_match.captured("value"))
                                .arg(o_match.captured("operand"));
            QString d = QString("#%1%2")
                                .arg(o_match.captured("operand"))
                                .arg(o_match.captured("value").remove("#"));
