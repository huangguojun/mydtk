...
dtkVisualizationWidgetsActorList::dtkVisualizationWidgetsActorList(QWidget *parent)
    : QFrame(parent), d(new dtkVisualizationWidgetsActorListPrivate)
{
...
void dtkVisualizationWidgetsActorList::update(void)
{
    ...
    for (int i = 0; i < actors->GetNumberOfItems(); i++) {
        if ((actor = vtkActor::SafeDownCast(actors->GetItemAsObject(i)))) {
            d->items << new QTreeWidgetItem(d, QStringList() << QString("Actor %1").arg(a_count++),
    ...
    for (int i = 0; i < volumes->GetNumberOfItems(); i++) {
        if ((volume = vtkVolume::SafeDownCast(volumes->GetItemAsObject(i)))) {
            d->items << new QTreeWidgetItem(d, QStringList() << QString("Volume %1").arg(v_count++),
                                            QTreeWidgetItem::UserType);
        }
    }
