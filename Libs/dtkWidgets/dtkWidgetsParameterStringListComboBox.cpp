...
// ///////////////////////////////////////////////////////////////////
// We define a BaseComboBox, which is just a QComboBox with an eventFilter to
// reject all scrolling events. That's it!
...
// The event filter kicks out every scroll event. We don't want these to be
// scrolled and when they're clicked they're not scrollable anyway.
...
dtkWidgetsParameterStringListComboBox::dtkWidgetsParameterStringListComboBox(QWidget *parent)
    : dtkWidgetsParameterBase<dtk::d_inliststring>(parent),
      d(new dtkWidgetsParameterStringListComboBoxPrivate)
{
...
bool dtkWidgetsParameterStringListComboBox::connect(dtkCoreParameter *p)
{
    ...
    QObject::connect(d->combo_box, &QComboBox::currentTextChanged,
                     [=](const QString &v) { m_parameter->shareValue(QVariant::fromValue(v)); });
