...
dtkWidgetsWorkspaceBarMenu::dtkWidgetsWorkspaceBarMenu(QVariantMap groups, QVariantMap group_descs,
                                                       QVariantMap works_descs, QWidget *parent)
    : QQuickWidget(parent)
{
...
public:
    void createWorkspace(const QColor &, const QString &, dtkWidgetsWorkspace *workspace,
...
void dtkWidgetsWorkspaceBarPrivate::onItemClicked(int index)
{
    ...
    if (index == q->count() - 1) {
        ...
        this->menu =
                new dtkWidgetsWorkspaceBarMenu(this->groups, this->group_descs, this->works_descs);
        this->menu->setAttribute(Qt::WA_DeleteOnClose, true);
        this->menu->resize(dtkApp->window()->width() * 1 / 2, dtkApp->window()->height() * 3 / 4);
        this->menu->move(q->parentWidget()->frameGeometry().topLeft()
                         + QPoint(q->parentWidget()->width() / 2 - this->menu->width() / 2,
                                  q->parentWidget()->height() / 2 - this->menu->height() / 2));
        this->menu->show();

        QObject *context = this->menu->rootObject();

        connect(context, SIGNAL(clicked(const QString &)), q,
                SLOT(createWorkspace(const QString &)));
    ...
    dtkWidgetsWorkspace *workspace =
            dynamic_cast<dtkWidgetsWorkspace *>(this->stack->widget(index));
...
dtkWidgetsWorkspaceBar::dtkWidgetsWorkspaceBar(QWidget *parent)
    : QTabBar(parent), d(new dtkWidgetsWorkspaceBarPrivate)
{
    d->q = this;
    ...
    QShortcut *nextWS = new QShortcut(
            QKeySequence(Qt::ControlModifier + Qt::ShiftModifier + Qt::Key_PageDown), this);
    QShortcut *prevWS = new QShortcut(
            QKeySequence(Qt::ControlModifier + Qt::ShiftModifier + Qt::Key_PageUp), this);
...
void dtkWidgetsWorkspaceBar::addWorkspaceInMenu(const QString &group,
                                                const QString &group_desciption,
                                                const QString &type,
                                                const QString &type_description,
                                                const QString &workspace_name, const QColor &color)
{
                                             ...
                                             bool display_destroy)
{
    ...
    if (!workspace) {
        qWarning() << "can't create workspace of type" << type
                   << dtk::widgets::workspace::pluginFactory().keys();
        return;
    }

    // the leave() must be done BEFORE changing d->current_workspace (which is
    // what d->createWorkspace DOES!!)
    if (d->current_workspace)
        d->current_workspace->leave();

    d->createWorkspace(d->workspace_colors[type], name, workspace,
                       display_destroy); // this does: d->current_workspace = workspace;
