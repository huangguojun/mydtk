...
dtkVisualizationPlot2D::dtkVisualizationPlot2D(QWidget *parent)
    : dtkVisualizationCanvas(parent), d(new dtkVisualizationPlot2DPrivate)
{
    static int count = 1;
    ...
    // dtkWidgetsMenu *menu = menu_bar->addMenu(fa::delicious, "View " +
    // this->objectName());
    ...
    // dtkWidgetsMenuItemDIY *fields_diy = new dtkWidgetsMenuItemDIY("fields
    // selection"); fields_diy->addLayout(fields_buttons_layout);
    ...
    // dtkWidgetsMenuItemDIY *range_diy = new dtkWidgetsMenuItemDIY("Range",
    // range_w); menu->addItem(range_diy);
    ...
    connect(d->field_y, &QListWidget::currentRowChanged, [=](int currentRow) {
        // it will happen if we call clear on the qlistwidget when a row is
        // selected
        ...
        if (d->save_field->isChecked()) {
            ...
            vtkAbstractArray *arr =
                    d->fields_table->GetColumnByName(row_text.toStdString().c_str());
            ...
            if (!d->old_fields->HasArray(new_name.toStdString().c_str())) {
                ...
                dtkVisualizationPlot2DPrivate::FieldParameters new_param =
                        d->fields_parameters[row_text];
                ...
                this->addFieldY(new_field, false, d->fields_parameters[row_text].chart_type,
        ...
        if (d->remove_field->isChecked()) {
            ...
            if (d->old_fields_param.contains(d->field_y->item(currentRow)->text())) {
                ...
                d->old_fields->RemoveArray(
                        d->field_y->item(currentRow)->text().toStdString().c_str());
            }
    ...
    connect(export_as_png, &QPushButton::clicked, [=]() {
        ...
        QFileDialog *dialog =
                new QFileDialog(this, tr("Save as"), path, QString("PNG file (*.png)"));
    ...
    connect(export_as_csv, &QPushButton::clicked, [=]() {
        ...
        QFileDialog *dialog =
                new QFileDialog(this, tr("Save as"), path, QString("csv file (*.csv)"));
        dialog->setAcceptMode(QFileDialog::AcceptSave);
        dialog->setFileMode(QFileDialog::AnyFile);
...
{
    vtkSmartPointer<vtkDelimitedTextWriter> text_writer =
    ...
    for (int row = 0; row < d->field_y->count(); ++row) {
        ...
        if (d->field_y->item(row)->checkState() == Qt::Checked) {
            dtkVisualizationPlot2DPrivate::FieldParameters param =
                    d->fields_parameters[d->field_y->item(row)->text()];

            vtkPlot *points = d->chart->AddPlot(param.chart_type);
            points->SetInputData(d->fields_table, d->field_x->currentText().toStdString(),
...
void dtkVisualizationPlot2D::addFieldY(vtkSmartPointer<vtkAbstractArray> field, bool checked,
                                       int chart_type, int marker_style, QColor color, int width)
{
    Q_ASSERT_X(chart_type >= 0 && chart_type < 7, "setChartType",
               "0: Line , 1: Points , 2: Bar , 3: Stacked , 4: Bag , 5: "
               "functionalBag , 6: Area  -> as in vtkPlot");
    Q_ASSERT_X(marker_style >= 0 && marker_style < 6, "setMarkerStyle",
...
void dtkVisualizationPlot2D::clearFields(void)
{
    ...
    QMap<QString, dtkVisualizationPlot2DPrivate::FieldParameters>::const_iterator i =
            d->old_fields_param.constBegin();
    while (i != d->old_fields_param.constEnd()) {
        this->addFieldY(d->old_fields->GetArray(i.key().toStdString().c_str()), false,
                        i.value().chart_type, i.value().marker_style, i.value().color);
        ++i;
    }
}
