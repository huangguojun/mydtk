...
dtkVisualizationDecoratorSnapshot::dtkVisualizationDecoratorSnapshot(void)
    : d(new dtkVisualizationDecoratorSnapshotPrivate())
{
    this->setObjectName("Snapshot");

    d->window_to_image_filter = vtkSmartPointer<vtkWindowToImageFilter>::New();
    // d->window_to_image_filter->SetMagnification(3); //set the resolution of
    // the output image (3 times the current resolution of vtk render window)
    d->window_to_image_filter->SetInputBufferTypeToRGBA(); // also record the alpha (transparency)
                                                           // channel
    ...
    connect(d->push_button, &QPushButton::clicked, [=]() {
        ...
        QString path = QFileDialog::getSaveFileName(nullptr, tr("Save snapshot"), ".",
                                                    tr("Images (*.png)"));
