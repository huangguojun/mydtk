...
dtkVisualizationViewHybridPrivate::dtkVisualizationViewHybridPrivate(QWidget *parent)
    : QVTKWidget(parent)
{
...
void dtkVisualizationViewHybridPrivate::resizeEvent(QResizeEvent *event)
{
    ...
    if (this->bottom_left_widget) {
        this->bottom_left_widget->move(
                10, event->size().height() - this->bottom_left_widget->sizeHint().height() - 50);
    }
    if (this->bottom_right_button) {
        this->bottom_right_button->move(event->size().width() - this->bottom_right_button->width()
                                                - 10,
                                        event->size().height() - 40);
    }
    if (this->bottom_right_widget) {
        this->bottom_right_widget->move(
                event->size().width() - this->bottom_right_widget->sizeHint().width() - 10,
                event->size().height() - this->bottom_right_widget->sizeHint().height() - 50);
    }
...
void dtkVisualizationViewHybridPrivate::addImageActors(vtkImageData *image)
{
    ...
    if (this->clut_editor) {
        ...
        color_function =
                static_cast<vtkColorTransferFunction *>(this->clut_editor->colorTransferFunction());
        if (!color_function)
    ...
    if (!this->image_color)
        this->image_color = vtkSmartPointer<vtkImageMapToColors>::New();
...
dtkVisualizationViewHybrid::dtkVisualizationViewHybrid(QWidget *parent) : QWidget(parent)
{
    ...
    // connect(d,
    // static_cast<void(dtkVisualizationViewHybridPrivate::*)(int)>(&dtkVisualizationViewHybridPrivate::sliceOrientationChanged),
    // this, &dtkVisualizationViewHybrid::sliceOrientationChanged);
    connect(d, &dtkVisualizationViewHybridPrivate::sliceOrientationChanged, this,
            &dtkVisualizationViewHybrid::sliceOrientationChanged);
...
{
    if (d->bottom_left_button)
        delete d->bottom_left_button;
...
{
    if (d->bottom_right_button)
        delete d->bottom_right_button;
...
void dtkVisualizationViewHybrid::setClutEditor(dtkVisualizationWidgetsClutEditor *clut_editor)
{
    ...
    QObject::connect(clut_editor, &dtkVisualizationWidgetsClutEditor::updated, [=]() {
        if (d->image_color) {
            d->image_color->SetLookupTable(static_cast<vtkColorTransferFunction *>(
        ...
        for (int i = 0; i < volumes->GetNumberOfItems(); ++i) {
            if ((volume = vtkVolume::SafeDownCast(volumes->GetItemAsObject(i)))) {
                volume->GetProperty()->SetColor(static_cast<vtkColorTransferFunction *>(
                        d->clut_editor->colorTransferFunction()));
                volume->GetProperty()->SetScalarOpacity(static_cast<vtkPiecewiseFunction *>(
...
{
    qDebug() << Q_FUNC_INFO
             << "to be implemented as an iteration over "
                "3D/2D+X/2D-X/2D+Y/2D-Y/2D+Z/@D-Z";
}
...
void dtkVisualizationViewHybrid::link(dtkVisualizationViewHybrid *other)
{
    ...
    other->d->GetRenderWindow()->AddObserver(vtkCommand::RenderEvent, this,
                                             &dtkVisualizationViewHybrid::render);
...
void dtkVisualizationViewHybrid::switchTo2DXY(void)
{
    ...
    if (d->viewer)
        d->setSliceOrientation(dtkVisualizationViewHybridPrivate::SLICE_ORIENTATION_XY);
...
void dtkVisualizationViewHybrid::switchTo2DXZ(void)
{
    ...
    if (d->viewer)
        d->setSliceOrientation(dtkVisualizationViewHybridPrivate::SLICE_ORIENTATION_XZ);
...
void dtkVisualizationViewHybrid::switchTo2DYZ(void)
{
    ...
    if (d->viewer)
        d->setSliceOrientation(dtkVisualizationViewHybridPrivate::SLICE_ORIENTATION_YZ);
