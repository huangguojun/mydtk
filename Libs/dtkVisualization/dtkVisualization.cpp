...
void initialize(const QString &s)
{
    ...
    if (path.isEmpty()) {
        ...
        // app->organization
        // user_space
        // ini format since setDefaultFormat is set in
    dtkApplicationPrivate::initialize QSettings settings; path =
    settings.value("dtk-visualization/plugins").toString(); if (path.isEmpty())
    { path = QDir::cleanPath(DTKVISUALIZATION_INSTALL_PREFIX +
    "/plugins/dtkVisualization"); dtkDebug() << Q_FUNC_INFO << "No plugin path
    configured, use default:" << path;
        }
    }
