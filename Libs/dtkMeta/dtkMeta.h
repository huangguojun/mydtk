...
template<typename T>
struct dtkMetaTypeHandler<T *, false>
{
    static bool canConvert(const QList<int> &types);
...
template<typename T>
struct dtkMetaTypeHandler<T *, true>
{
    static bool canConvert(const QList<int> &types);
    static QVariant variantFromValue(T *t);
    static T *clone(T *t);
...
struct dtkMetaTypeHandlerHelper;

template<typename T>
struct dtkMetaTypeHandlerHelper<T *, false>
{
    static bool canConvert(const QList<int> &types);
    static T *clone(T *t);
    static void copy(T *source, T *target);
...
template<typename T>
struct dtkMetaTypeHandlerHelper<T *, true>
{
    static bool canConvert(const QList<int> &types);
    static T *clone(T *);
    static void copy(T *source, T *target);
...
public:
    ...
    static bool canConvert(const QList<int> &types);
    template<typename T>
    static QVariant variantFromValue(const T &t);
    template<typename T>
    static QVariant variantFromValue(T *t);
    template<typename T>
    static T *clone(T *t);
    ...
    static void copy(T *source, T *target);
    template<typename T>
    static bool registerContainerPointerConverter(int id);
...
template<typename T>
QDataStream &operator<<(QDataStream &s, T *t);
template<typename T>
QDataStream &operator>>(QDataStream &s, T *&t);
...
template<typename T>
QDataStream &operator<<(QDataStream &s, const std::list<T *> &l);
template<typename T>
QDataStream &operator>>(QDataStream &s, std::list<T *> &l);
...
#define DTK_DECLARE_SEQUENTIAL_CONTAINER_POINTER(CONTAINER_ARG)                                    \
    ...
    {                                                                                              \
        ...
        {                                                                                          \
            ...
            const int newId = qRegisterNormalizedMetaType<CONTAINER_ARG<T> *>(                     \
                    typeName, reinterpret_cast<CONTAINER_ARG<T> **>(quintptr(-1)));                \
    ...
    template<typename T>                                                                           \
    struct dtkMetaTypeIsSequentialContainerPointer<CONTAINER_ARG<T> *> : std::true_type            \
    {                                                                                              \
    };
