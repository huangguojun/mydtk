  ...
  , ShouldStop(false)
{
  ...
  this->ForwardedSocket.open(
    ForwardEndpoint.protocol()); // Opening the socket with an UDP v4 protocol
  ...
  const boost::system::error_code& error, std::size_t numberOfBytes)
{
  ...
  NetworkPacket* packet = NetworkPacket::BuildEthernetIP4UDP(
    this->RXBuffer, numberOfBytes, sourceIP, sourcePort, ourPort);
  ...
  if (this->isForwarding)
  {
    ForwardedSocket.send_to(
      boost::asio::buffer(packet->GetPayloadData(), packet->GetPayloadSize()), ForwardEndpoint);
  }
