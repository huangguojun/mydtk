                                                  ...
                                                  unsigned short destinationPort)
{
    ...
    std::copy(payload, payload + payloadSize,
              packet->PacketData.begin() + sizeof(NetworkPacket::EthIP4UDPHeaderDefault));

    // Set IP-frame length (which is payloadSize + 28), in network order: big
    // endian
    ...
    // Set UDP-frame length (which is payloadSize + 8), in network order: big
    // endian
    ...
    std::copy(sourceIPv4BigEndian, sourceIPv4BigEndian + 4,
