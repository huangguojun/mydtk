...
int vtkLidarReader::ReadFrameInformation()
{
    ...
    if (this->FrameCatalog.size() > 0) {
        ...
        for (size_t i = 0; i < this->FrameCatalog.size(); i++) {
            diffs[i] = this->FrameCatalog[i].FirstPacketDataTime
                    - this->FrameCatalog[i].FirstPacketNetworkTime;
        }
...
void vtkLidarReader::SetTimestepInformation(vtkInformation *info)
{
    ...
    if (this->FrameCatalog.size()) {
        ...
        info->Set(vtkStreamingDemandDrivenPipeline::TIME_STEPS(), firstTimestepPointer,
                  numberOfTimesteps);
...
{
    // iterating over all timesteps until finding the first one with a greater
    // time value
    auto idx = std::lower_bound(
            this->FrameCatalog.begin(), this->FrameCatalog.end(), packetTime,
...
int vtkLidarReader::GetFrameIndexForDataTime(double dataTime)
{
    ...
    // time value
    auto idx = std::lower_bound(
            this->FrameCatalog.begin(), this->FrameCatalog.end(), dataTime,
...
void vtkLidarReader::Open()
{
    ...
    if (!this->Reader->Open(this->FileName, filterPCAP.c_str())) {
        vtkErrorMacro(<< "Failed to open packet file: " << this->FileName << "!\n"
                      << this->Reader->GetLastError()) this->Close();
    }
}
...
{
    if (!this->Reader) {
        vtkErrorMacro("SaveFrame() called but packet file reader is not open.") return;
    }
    ...
    // because fpos_t is plateform specific and should not be used for
    // comparaison it's not possible to simply interate from FiePositions[start]
    // to FilePositions[end] we need to detect new frame in the pcap directly
    // once again
    ...
    // If '[]' represents a packet, '|' represents the separation between
    // frames, Then the most generic form of a Lidar PCAP is*:
    ...
    // A possible improvement to LidarView would be to not always hide this
    // first frame, depending on a flag set on the reader. *if you are very
    // lucky the first frame will start at the begining of the first packet, and
    // there will be no "incomplete frame".
    //
    // In my test, writing all frames of the PCAP results in a .pcap file
    // exactly identical to the one that is read, if you enable
    ...
    while (this->Reader->NextPacket(data, dataLength, timeSinceStart, &header,
                                    &dataHeaderLength)
                                ...
                                vtkInformationVector *outputVector)
{
    ...
    if (this->FrameCatalog.size()
        == 1) // This mean that the reader did not manage to parser the pcap file
    {
        return 1;
    }
    ...
    // time value
    auto idx = std::lower_bound(
            this->FrameCatalog.begin(), this->FrameCatalog.end(), timestep,
            [](FrameInformation &fp, double d) { return fp.FirstPacketNetworkTime < d; });

    auto frameRequested = std::distance(this->FrameCatalog.begin(), idx);

    if (idx == this->FrameCatalog.end()) {
        vtkErrorMacro("Cannot meet timestep request: "
                      << frameRequested << ".  Have " << this->GetNumberOfFrames() << " datasets.");
        return 0;
    }
...
//-----------------------------------------------------------------------------
int vtkLidarReader::RequestInformation(vtkInformation *request, vtkInformationVector **inputVector,
                                       vtkInformationVector *outputVector)
{
