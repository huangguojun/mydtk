...
{
public:
    vtkTypeMacro(vtkLidarPacketInterpreter, vtkAlgorithm) void PrintSelf(ostream &os,
                                                                         vtkIndent indent)
    {
    }
    ...
    /**
     * @brief LoadCalibration read a provided calibration file to initialize the
     * sensor's calibration parameters (angles corrections, distances
     * corrections, ...) which will be used later on while processing the
     * packet.
    ...
    /**
     * @brief GetCalibrationTable return a table conttaining all information
     * related to the sensor calibration.
     */
    virtual vtkSmartPointer<vtkTable> GetCalibrationTable() { return this->CalibrationData.Get(); }

    /**
     * @brief ProcessPacket process the data packet to create incrementaly the
     * frame. Each time a packet is processed by the function, the points which
     * are encoded in the packet are decoded using the calibration information
     * and add to the frame. A warning should be raise in case the calibration
     * information does not match the data (ex: factory field, number of laser,
     * ...)
     * @param data raw data packet
     * @param bytesReceived size of the data packet
     * @param startPosition offset in the data packet used when a frame start in
     * the middle of a packet
     */
    virtual void ProcessPacket(unsigned char const *data, unsigned int dataLength) = 0;

    /**
     * @brief SplitFrame take the current frame under construction and place it
     * in another buffer
    ...
    /**
     * @brief CreateNewEmptyFrame construct a empty polyData with the right
     * DataArray and allocate some space. No CellArray should be created as it
     * can be create once the frame is ready.
     * @param numberOfPoints indicate the space to allocate @todo change the
     * meaning
     */
    virtual vtkSmartPointer<vtkPolyData>
    CreateNewEmptyFrame(vtkIdType numberOfPoints, vtkIdType prereservedNumberOfPoints = 0) = 0;

    /**
     * @brief PreProcessPacket is use to construct the frame index and get some
     * corretion or live calibration comming from the data. A warning should be
     * raise in case the calibration information does not match the data (ex:
     * factory field, number of laser, ...)
     * @param data raw data packet
    ...
    /**
     * @brief ResetCurrentFrame reset all information to handle some new frame.
     * This reset the frame container, some information about the current frame,
     * guesses about the sensor type, etc
     */
    ...
    /**
     * @brief GetLastFrameAvailable return the last frame that have been process
     * completely
     * @return
     */
    ...
    vtkGetMacro(CalibrationFileName, std::string) vtkSetMacro(CalibrationFileName, std::string)

            vtkGetMacro(CalibrationReportedNumLasers, int)
                    vtkSetMacro(CalibrationReportedNumLasers, int)

                            vtkGetMacro(IsCalibrated, bool) vtkSetMacro(IsCalibrated, bool)

                                    vtkGetMacro(TimeOffset, double) vtkSetMacro(TimeOffset, double)
    ...
    virtual void GetLaserSelection(bool *v)
    {
        std::copy(this->LaserSelection.begin(), this->LaserSelection.end(), v);
    }
    ...
    vtkGetMacro(DistanceResolutionM, double) vtkSetMacro(DistanceResolutionM, double)

            vtkGetMacro(Frequency, double) vtkSetMacro(Frequency, double)

                    vtkGetMacro(IgnoreZeroDistances, bool) vtkSetMacro(IgnoreZeroDistances, bool)

                            vtkGetMacro(IgnoreEmptyFrames, bool) vtkSetMacro(IgnoreEmptyFrames,
                                                                             bool)

                                    vtkGetMacro(ApplyTransform, bool) vtkSetMacro(ApplyTransform,
                                                                                  bool)

                                            vtkGetObjectMacro(
                                                    SensorTransform,
                                                    vtkTransform) virtual void SetSensorTransform(vtkTransform
                                                                                                          *);

    vtkGetMacro(CropMode, int) vtkSetMacro(CropMode, int)

            vtkGetMacro(CropOutside, bool) vtkSetMacro(CropOutside, bool)
...
protected:
    ...
    //! Number of laser which can be shoot at the same time or at least in dt <
    //! epsilon
    ...
    //! Laser distance resolution (quantum) which also correspond to the points
    //! precision
    double DistanceResolutionM = 0;

    //! Frequency at which a new frame is obtain. It can be constant or variable
    //! depending on the lidar. Note that it is note necessarily expressed in
    //! Hz, in case of rotational lidar, this correspond to the RPM (Rotation
    //! Per Minute).
    double Frequency = 0;
    ...
    //! If true, the region outside of the area defined by CropRegion is
    //! cropped/removed. If false, the region within the area is
    //! cropped/removed.
    ...
    //! - Spherical -> [azimuth_min, azimuth_max, vertAngle_min, vertAngle_max,
    //! r_min, r_max] (0° <= azimuth <= 360° (mod 360°), -90° <= vertAngle <=
    //! 90°, r >= 0.0, vertAngle increasing with z and vertAngle = 0 in plane z
    //! = 0) any interval is valid for the azimuthal but [ a, b ] is different
    //! from [ b, a ]
