...
hid_t &
dtkIoDataModelPHdf5Private::datasetId(const QString &dataset_name,
                                      ...
                                      const int &dim = -1, quint64 *shape = nullptr)
{
    ...
    if (!dataset_hash.contains(dataset_name)) {
        if (dim > 0) {
            ...
            // erase first (root) and last (dataset) elements to only keep
            // groups
            ...
            if (dataset_name_split.length() > 0) {
                ...
                for (auto it : dataset_name_split) {
                    ...
                    if (!group_hash.contains(path)) {
                        if (H5Lexists(file_id, path.toUtf8().constData(),
                                      H5P_DEFAULT)) // The group exists
                            group_hash[path] =
                                    H5Gopen(file_id, path.toUtf8().constData(), H5P_DEFAULT);
                        else
                            group_hash[path] = H5Gcreate(file_id, path.toUtf8().constData(),
            ...
            case dtkIoDataModel::Int: {
                dataset_hash[dataset_name] =
                        H5Dcreate(file_id, dataset_name.toUtf8().constData(), H5T_NATIVE_INT,
                                  dataspace, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
                break;
            }
            case dtkIoDataModel::LongLongInt: {
                dataset_hash[dataset_name] =
                        H5Dcreate(file_id, dataset_name.toUtf8().constData(), H5T_NATIVE_LLONG,
                                  dataspace, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
                break;
            }
            case dtkIoDataModel::Double: {
                dataset_hash[dataset_name] =
                        H5Dcreate(file_id, dataset_name.toUtf8().constData(), H5T_NATIVE_DOUBLE,
                                  dataspace, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
                break;
            }
            default:
            ...
            dataset_hash[dataset_name] =
                    H5Dopen(file_id, dataset_name.toUtf8().constData(), H5P_DEFAULT);
        }
    }
...
{
    if (d->file_is_open) {
        qDebug() << " ERROR set the communicator before opening a file in "
                    "dtkIoDataModelPHdf5";
        dtkError() << " ERROR set the communicator before opening a file in "
                      "dtkIoDataModelPHdf5";
    } else
        d->comm = comm;
}
...
void dtkIoDataModelPHdf5::fileOpen(const QString &file_name, FileMode mode)
{
    ...
    if (!d->file_is_open) {
        ...
        case dtkIoDataModel::Trunc:
            d->file_id = H5Fcreate(file_name.toUtf8().constData(), H5F_ACC_TRUNC, H5P_DEFAULT,
                                   d->prop_list_id);

            break;
        case dtkIoDataModel::NotExisting:
            d->file_id = H5Fcreate(file_name.toUtf8().constData(), H5F_ACC_EXCL, H5P_DEFAULT,
                                   d->prop_list_id);
            break;
        case dtkIoDataModel::ReadOnly:
            d->file_id = H5Fopen(file_name.toUtf8().constData(), H5F_ACC_RDONLY, d->prop_list_id);
            break;
        case dtkIoDataModel::ReadWrite:
            d->file_id = H5Fopen(file_name.toUtf8().constData(), H5F_ACC_RDWR, d->prop_list_id);
            break;
        default:
        ...
        if (d->file_id < 0) {
            ...
            // if the file is correctly open, create a propery list to
            // collectively write datasets
            d->file_is_open = true;
...
void dtkIoDataModelPHdf5::read(const QString &dataset_name, DataType type, void *values)
{
    ...
    case dtkIoDataModel::LongLongInt: {
        H5Dread(d->datasetId(dataset_name), H5T_NATIVE_LLONG, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                values);
        break;
    }
    case dtkIoDataModel::Double: {
        H5Dread(d->datasetId(dataset_name), H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                values);
        break;
    }
    default:
 ...
 * dimensions
 */
void dtkIoDataModelPHdf5::write(const QString &dataset_name, DataType type, int dimension,
                                quint64 *shape, void *values)
{
    d->status = 0;
    ...
    if (values) {
        ...
        case dtkIoDataModel::Int: {
            d->status =
                    H5Dwrite(dataset_id, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, d->prop_list_id, values);
            break;
        }
        case dtkIoDataModel::LongLongInt: {
            d->status = H5Dwrite(dataset_id, H5T_NATIVE_LLONG, H5S_ALL, H5S_ALL, d->prop_list_id,
                                 values);
            break;
        }
        case dtkIoDataModel::Double: {
            d->status = H5Dwrite(dataset_id, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, d->prop_list_id,
                                 values);
            break;
        }
        default:
...
// hyperslab write
void dtkIoDataModelPHdf5::writeHyperslab(const QString &dataset_name, DataType type,
                                         quint64 *offset, quint64 *stride, quint64 *count,
                                         quint64 *block, quint64 *values_shape, void *values)
{
    ...
    // set the dimensions of values. memory dataspace and the selection within
    // it
    hid_t values_dataspace =
            H5Screate_simple(H5Sget_simple_extent_ndims(file_dataspace), values_shape, NULL);

    switch (type) {
    case dtkIoDataModel::Int:
        d->status = H5Dwrite(dataset_id, H5T_NATIVE_INT, values_dataspace, file_dataspace,
                             d->prop_list_id, values);
        break;
    case dtkIoDataModel::LongLongInt:
        d->status = H5Dwrite(dataset_id, H5T_NATIVE_LLONG, values_dataspace, file_dataspace,
                             d->prop_list_id, values);
        break;
    case dtkIoDataModel::Double:
        d->status = H5Dwrite(dataset_id, H5T_NATIVE_DOUBLE, values_dataspace, file_dataspace,
                             d->prop_list_id, values);
        break;
    default:
        dtkError() << "write method: Datatype not supported";
...
// assuming dataset and values have the same shape and the points values are
// contiguous in values
void dtkIoDataModelPHdf5::writeByCoord(const QString &dataset_name, DataType type,
                                       quint64 nb_points, quint64 *points_coord, void *values)
{
    d->status = 0;
    hid_t dataset_id = d->datasetId(dataset_name);
