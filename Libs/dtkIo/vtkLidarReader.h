...
//! @todo a decition should be made if the opening/closing of the pcap should be
//! handle by the class itself of the class user. Currently this is not clear
...
public:
    ...
    {
        return this->FrameCatalog.size();
    }

    /**
     * @copydoc FileName
     */
    vtkGetMacro(FileName, std::string) virtual void SetFileName(const std::string &filename);
     ...
     * @todo a decition should be made if the opening/closing of the pcap should
     * be handle by the class itself of the class user. Currently this is not
     * clear
     */
    virtual void Open();
    ...
    /**
     * @brief SaveFrame save the packet corresponding to the desired frames in a
     * pcap file. Because we are saving network packet, part of previous and/or
     * next frames could be included in generated the pcap
    ...
    vtkGetMacro(ShowFirstAndLastFrame, bool) vtkSetMacro(ShowFirstAndLastFrame, bool)

            int GetLidarPort() override
    {
        return this->LidarPort;
    }
...
protected:
    ...
    //! Show/Hide the first and last frame that most of the time are partial
    //! frames
    bool ShowFirstAndLastFrame = false;

    //! libpcap wrapped reader which enable to get the raw pcap packet from the
    //! pcap file
...
private:
     ...
     * In case the calibration is contained in the pcap file, this will also
     * read it
     */
