...
namespace dtk {
    ...
    using parameter_arithmetic = std::enable_if_t<std::is_arithmetic<U>::value,
V>; template <typename U, typename V = void> using parameter_not_arithmetic =
std::enable_if_t<!std::is_arithmetic<U>::value, V>; template <typename U,
typename V = void> using is_core_parameter =
std::enable_if_t<std::is_base_of<dtkCoreParameter,
std::remove_pointer_t<std::decay_t<U>>>::value, V>;
}
...
#pragma mark - Connection management
    ...
    void sync(void);
    template<typename F>
    QMetaObject::Connection connect(F);
    void disconnect(void);

    void syncFail(void);
    template<typename F>
    QMetaObject::Connection connectFail(F);
...
public:
    ...
    dtkCoreParameterNumeric(const QString &, const T &, const T &, const T &,
                            const QString &doc = QString());
#ifndef SWIG
    template<typename U = T, typename = std::enable_if_t<std::is_floating_point<U>::value>>
    dtkCoreParameterNumeric(const QString &, const T &, const T &, const T &, const int &,
                            const QString &doc = QString());
#endif
    template<typename U = T>
    dtk::parameter_arithmetic<U, dtkCoreParameterNumeric &> operator=(const U &);
    template<typename U>
    dtk::is_core_parameter<U, dtkCoreParameterNumeric &> operator=(const U *);
    dtkCoreParameterNumeric &operator=(const QVariant &);
    dtkCoreParameterNumeric &operator=(const dtkCoreParameterNumeric &);
    template<typename U>
    dtk::parameter_arithmetic<U, dtkCoreParameterNumeric &>
    operator=(const dtkCoreParameterNumeric<U> &);
    ...
    template<typename U = T>
    std::enable_if_t<std::numeric_limits<U>::is_modulo && std::numeric_limits<T>::is_modulo,
                     dtkCoreParameterNumeric &>
    operator%=(const U &);
    template<typename U = T>
    std::enable_if_t<std::numeric_limits<U>::is_modulo, dtkCoreParameterNumeric &>
    ...
    template<typename U = T>
    dtk::parameter_arithmetic<U, bool> operator==(const U &);
    bool operator==(const QVariant &);
    template<typename U>
    dtk::parameter_arithmetic<U, bool> operator==(const dtkCoreParameterNumeric<U> &);
    ...
    template<typename U = T>
    std::enable_if_t<std::is_floating_point<U>::value> setDecimals(const int &);
...
protected:
    ...
    int m_decimals =
            std::numeric_limits<T>::max_digits10 / 1.75; // 9 decimals for double, 5 for float
};
...
public:
    ...
    dtkCoreParameterInList(const QString &, const T &, const QList<T> &,
...
public:
    ...
    dtkCoreParameterRange(const QString &, const std::array<T, 2> &, const T &, const T &,
                          const QString &doc = QString());
#ifndef SWIG
    template<typename U = T, typename = std::enable_if_t<std::is_floating_point<U>::value>>
    dtkCoreParameterRange(const QString &, const std::array<T, 2> &, const T &, const T &,
                          const int &, const QString &doc = QString());
#endif
