...
// is_instantiable : detects whether an object of the class can be instantiated
// or not
namespace detail {
template<typename T>
struct is_instantiable_impl
    : std::conditional_t<!std::is_abstract<std::remove_pointer_t<std::decay_t<T>>>::value,
                         std::is_constructible<std::remove_pointer_t<std::decay_t<T>>>,
                         std::false_type>
{
};
} // namespace detail
...
namespace detail {
template<typename T>
struct is_clonable_impl
    : std::conditional_t<dtk::is_instantiable<T>::value,
                         std::is_copy_constructible<std::remove_pointer_t<std::decay_t<T>>>,
                         std::false_type>
{
};
} // namespace detail
...
// is_numeric : detects whether the arithmetic operations +, -, *, / can be
// applied or not
namespace detail {
template<typename T>
...
// is_assignable : detects whether the type supports assignment operator = or
// not
namespace detail {
template<typename T>
struct is_assignable_impl
    : std::conditional_t<dtk::is_numeric<T>::value, std::true_type, std::is_copy_assignable<T>>
{
};
} // namespace detail
...
// is_bidirectional : detects whether the category of the iterator is
// bidirectional_iterator_tag or not
namespace detail {
template<typename T>
struct is_bidirectional_impl : std::is_same<typename std::iterator_traits<T>::iterator_category,
...
// is_random_access : detects whether the category of the iterator is
// random_access_iterator_tag or not
namespace detail {
template<typename T>
struct is_random_access_impl : std::is_same<typename std::iterator_traits<T>::iterator_category,
...
// is_sequential_container_pointer : detects whether the type is a pointer to a
// sequential container or not
template<typename T>
struct is_sequential_container_pointer : std::false_type
{
};

// is_associative_container_pointer : detects whether the type is a pointer to
// an associative container or not
template<typename T>
struct is_associative_container_pointer : std::false_type
{
};

// is_container_value_type_meta_type : detects whether the value type of a
// container is registered to the QMetaType system or not
namespace detail {
template<typename T>
struct is_container_value_type_meta_type_impl
    : std::conditional_t<QMetaTypeId2<typename std::remove_pointer_t<T>::value_type>::Defined,
